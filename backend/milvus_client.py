    ...
    def init_collection(self, dense_dim: int | None = None):
        ...
        def _init(client: MilvusClient) -> None:
            if not client.has_collection(self.collection_name):
                ...
                schema.add_field("subject", DataType.VARCHAR, max_length=100)
                schema.add_field("grade", DataType.VARCHAR, max_length=100)
                schema.add_field("book_version", DataType.VARCHAR, max_length=100)
                schema.add_field("resource_type", DataType.VARCHAR, max_length=50)
                schema.add_field("section_title", DataType.VARCHAR, max_length=255)
                schema.add_field("knowledge_tags", DataType.JSON)
        ...
        for hits in results:
            for hit in hits:
                formatted_results.append({
                    ...
                    "grade": hit.get("grade", ""),
                    "book_version": hit.get("book_version", ""),
                    "resource_type": hit.get("resource_type", ""),
                    "section_title": hit.get("section_title", ""),
                    "knowledge_tags": hit.get("knowledge_tags", []),
        ...
        for hits in results:
            for hit in hits:
                formatted_results.append({
                    ...
                    "subject": hit.get("entity", {}).get("subject", ""),
                    "grade": hit.get("entity", {}).get("grade", ""),
                    "book_version": hit.get("entity", {}).get("book_version", ""),
                    "resource_type": hit.get("entity", {}).get("resource_type", ""),
                    "section_title": hit.get("entity", {}).get("section_title", ""),
                    "knowledge_tags": hit.get("entity", {}).get("knowledge_tags", []),
